...
Isolated Claude Code sessions in Docker containers. The trust boundary is between the tier instance and the host — not between sessions. Each tier runs as a single long-lived tier instance (a Docker container on the local backend). Interactive sessions use `dtach` for inner persistence and run inline in whatever shell/pane the user invoked cbox from — same keybindings, same workflow, just containerized underneath (ADR 016). Autonomous sessions use container tmux (headless). All sessions share Docker, auth state, and `.claude.json`. Git isolation is per-session via worktrees.

## Architecture

```
Host tmux (or any shell)
├── Pane A: local shell
├── Pane B: cbox auth-fix       → SSH → dtach → /workspace/auth-fix
├── Pane C: cbox experiment     → SSH → dtach → /workspace/experiment
├── Pane D: cbox auth-fix       → ancillary shell in /workspace/auth-fix
│
...
Each `cbox <name>` runs inline in the invoking pane (ADR 016). Users
who want a dedicated tmux window or split can compose one themselves
(e.g. `tmux new-window cbox myfix` or `tmux split-window cbox myfix`).
...
- **Interactive sessions use dtach** — the first `cbox <name>` call creates a dtach session and runs ssh inline in the current pane (ADR 016). `cbox destroy` removes the socket. Tier instance lifecycle is separate.
...
| Command | Description |
|---------|-------------|
| `cbox <name> [project-or-path]` | Create or attach (idempotent). New: ssh+dtach inline in current pane, launching Claude. Existing: ancillary shell inline in same workspace. `--shell`/`--claude`/`--attach` to override. Inline always (ADR 016) — wrap with `tmux new-window` / `tmux split-window` yourself if you want a dedicated window or split. |
...
| `cbox tier reset <tier> [--yes]` | Wipe tier state (instance + `.claude` volume), keep image. Prompts unless `--yes`. |
| `cbox tier destroy <tier> [--yes]` | Full teardown: reset + remove tier image + on-host workspaces. Prompts unless `--yes`. |
...
`cbox <name>` is idempotent — first call starts Claude in dtach, subsequent calls open a shell in the same workspace. Everything runs inline in the invoking pane (ADR 016). `--shell` on first call for a shell instead of Claude; `--claude` on subsequent calls for another Claude session; `--attach` to reattach to the existing dtach session. `run` always detaches. Tier containers auto-start on first use if not running.
...
6. Runs ssh inline in the current shell/pane, taking over until the user detaches or exits

On subsequent calls (`cbox auth-fix`), detects the existing dtach socket and opens a shell in the same container/workspace — also inline. The Claude session is still running in the original dtach — use `--attach` to reattach to it directly (which replaces the current pane with the live agent). Use `--claude` to start an additional Claude session instead of a shell.
...
`cbox destroy auth-fix` destroys the session (dtach socket). The session workspace persists by default; pass `--workspace` to remove it. If no alive sessions remain in the tier, the tier instance auto-pauses.
...
Interactive sessions live inside host tmux as panes. No container tmux for interactive use — `dtach` provides inner persistence with zero keybinding footprint. Same prefix, same copy mode, same splits.

`cbox <name>` runs inline in whichever shell/pane invoked it (ADR 016) — it behaves like running `claude` directly, with no windowing side effects. Splits compose naturally: split the pane, run `cbox <name>` again, get an ancillary shell in the same workspace. Want a dedicated window? `tmux new-window cbox <name>`. cbox doesn't manage host tmux windows for sessions.

```
cbox auth-fix       → SSH → dtach (Claude) inline in current pane
ctrl-b d            → detach from host tmux (dtach keeps Claude alive)
cbox auth-fix       → ancillary shell in same workspace, inline in current pane
cbox auth-fix --attach → reattach to the dtach session inline in current pane
cbox destroy auth-fix → destroy session (socket); workspace persists
```
...
**Visual indicator**: each cbox session emits an OSC 0 escape (`\e]0;cbox:<session>\a`) before launching the inner command, so the terminal/tmux title reflects the session regardless of which pane it lands in.
...
cbox/
  ...
  src/
    ...
    tmux.rs           # host tmux helpers (used by `cbox auth`)
...
- Run SSH + dtach inline in the invoking pane (ADR 016)
- Idempotent: subsequent calls open new shell in same workspace, inline
- Kill dtach session on destroy
- **Result:** can create, connect, destroy sessions from any shell or tmux pane
...
2. `cbox test dev --tier dev` → tier instance starts, dtach session running, ssh attached inline in the current pane
3. `cbox test` from another pane → second independent shell in same workspace, inline in that pane
4. `cbox second dev` → second session in same container, separate workspace
5. Both sessions share `.claude.json`, Docker daemon, auth state
6. Git workspaces isolated: `/workspace/test/` and `/workspace/second/`
7. Kill SSH connection → dtach keeps process alive → reconnect succeeds
8. `docker compose up` in two sessions with `COMPOSE_PROJECT_NAME` → separate stacks, no conflicts
9. Dev tier settings.json enforced: domain allowlist, tool permissions
10. `cbox run hello --tier auto "create hello.txt"` → runs in auto tier, detached (container tmux)
11. `cbox list` → shows sessions across tiers (reads `/run/cbox/` in each container)
12. `cbox destroy test && cbox destroy second` → sessions destroyed, tier instance auto-pauses
13. `cbox tier stop dev` → tier instance stopped
