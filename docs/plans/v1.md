...
Two paths, selected per-tier via `tiers.<name>.auth` (ADR 018):

- **`auth: api-key`** (default, backwards-compatible). Inject `ANTHROPIC_API_KEY` (resolved via `op read` on host) at container start. Skips Claude Code's on-disk OAuth credential file (`~/.claude/.credentials.json`) entirely — nothing Anthropic-related lands on the tier volume. Recommended for headless / CI tiers and `cbox run`.
- **`auth: oauth`**. Claude Code uses its native `~/.claude/.credentials.json` on the per-tier **claude state volume** (ADR 014). The user runs `cbox auth <tier>` once to land a subscription-billed OAuth token; cbox never reads, mints, or rotates it. Recommended for interactive tiers on the user's laptop. cbox suppresses any Anthropic env credential listed on the tier in this mode.
